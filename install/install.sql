...
--
-- Client: localhost
-- Généré le: Ven 27 Décembre 2013 à 10:32
...
CREATE TABLE IF NOT EXISTS `chat` (
  ...
  `datetime` datetime DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=15997 ;
...
CREATE TABLE IF NOT EXISTS `chatHistory` (
  ...
  `datetime` datetime DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=MEMORY  DEFAULT CHARSET=utf8 AUTO_INCREMENT=337 ;
...
CREATE TABLE IF NOT EXISTS `macro` (
  ...
  `execute_cmd_id` int(11) NOT NULL,
  `order` int(11) NOT NULL,
  `option` text,
  PRIMARY KEY (`execute_cmd_id`,`order`,`macroCmd_id`),
  KEY `fk_command_has_command_cmd2_idx` (`execute_cmd_id`),
  KEY `fk_command_has_command_cmd1_idx` (`macroCmd_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE IF NOT EXISTS `view` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `name_UNIQUE` (`name`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='<double-click to overwrite multiple objects>' AUTO_INCREMENT=19 ;
...
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='<double-click to overwrite multiple objects>' AUTO_INCREMENT=388 ;
...
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='<double-click to overwrite multiple objects>' AUTO_INCREMENT=153 ;
...
--
ALTER TABLE `macro`
  ADD CONSTRAINT `fk_macro_cmd1` FOREIGN KEY (`macroCmd_id`) REFERENCES `cmd` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `fk_macro_cmd2` FOREIGN KEY (`execute_cmd_id`) REFERENCES `cmd` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;
