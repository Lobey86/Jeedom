<?php

/* This file is part of Jeedom.
 *
 * Jeedom is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * Jeedom is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
 * GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License
 * along with Jeedom. If not, see <http://www.gnu.org/licenses/>.
 */

try {
    require_once(dirname(__FILE__) . '/../../core/php/core.inc.php');
    include_file('core', 'authentification', 'php');

    if (!isConnect()) {
        throw new Exception('401 Unauthorized');
    }

    if (init('action') == 'getModuleConf') {
        $module = new module(init('modulePath'));
        $return = array();
        $return['id'] = $module->getId();
        $return['name'] = $module->getName();
        $return['description'] = $module->getDescription();
        $return['licence'] = $module->getLicence();
        $return['author'] = $module->getAuthor();
        $return['require'] = $module->getRequire();
        $return['installation'] = $module->getInstallation();
        $return['version'] = $module->getVersion();
        $return['category'] = $module->getCategory();
        $return['filepath'] = $module->getFilepath();
        $return['activate'] = $module->isActive();
        $return['configurationPath'] = $module->getPathToConfigurationById();
        $return['checkVersion'] = version_compare(getVersion('jeedom'), $module->getRequire());
        ajax::success($return);
    }

    if (init('action') == 'toggleModule') {
        $module = new module(init('id'));
        if (!is_object($module)) {
            throw new Exception('Module introuvable : ' . init('id'));
        }
        $module->setIsEnable(init('state'));
        ajax::success();
    }


    throw new Exception('Aucune methode correspondante à : ' . init('action'));
    /*     * *********Catch exeption*************** */
} catch (Exception $e) {
    ajax::error(displayExeption($e), $e->getCode());
}
?>
