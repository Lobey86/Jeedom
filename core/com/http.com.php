        ...
        while ($retry) {
            ...
            if (curl_errno($ch)) {
                if ($_logErrorIfNoResponse) {
                    log::add('http.com', 'Error', 'Echec curl : ' . curl_error($ch) . ' sur la commande ' . $this->url . ' relance ' . $nbRetry);
                }
                if ($nbRetry <= $_maxRetry) {
                    ...
                    if ($_logErrorIfNoResponse) {
                        log::add('http.com', 'Error', 'Erreur curl : ' . curl_error($ch) . ' sur la commande ' . $this->url);
                    }
