...
class dataStore {
    ...
    public function preSave() {
        ...
        if (!in_array($this->getType(), $allowType)) {
            throw new Exception(translate::sentence('Le type doit être un des suivants : ',__FILE__) . print_r($allowType, true));
        }
        if (!is_numeric($this->getLink_id())) {
            throw new Exception(translate::sentence('Link_id doit etre un chiffre',__FILE__));
        }
        if ($this->getKey() == '') {
            throw new Exception(translate::sentence('La clef ne peut etre vide',__FILE__));
        }
        if ($this->getId() == '') {
