    ...
    public function execute(&$_scenario, $_initialScenarioState) {
        ...
        if ($this->getSubtype() == 'condition') {
            foreach ($this->getExpression() as $expression) {
                return $expression->execute($_scenario, $_initialScenarioState);
            }
        }
        if ($this->getSubtype() == 'action') {
            $doCmd = true;
            if ($this->getOptions('runingScenario') == 1) {
                if ($_scenario->getIsRepeat() == 0 && $_initialScenarioState == 'on') {
                    $doCmd = false;
                } else {
                ...
                if ($_scenario->getIsRepeat() == 0 && $_initialScenarioState == 'stop') {
                    $doCmd = false;
                }
                $_scenario->setState('stop');
            }
            foreach ($this->getExpression() as $expression) {
                $return = $expression->execute($_scenario, $_initialScenarioState, $doCmd);
            }
            return $return;
        }
    }
