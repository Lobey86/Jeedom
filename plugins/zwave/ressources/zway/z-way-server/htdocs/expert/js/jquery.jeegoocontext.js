...
(function($){
    ...
    // Before delay, don't select option when mouse didnt move nor hide menu when mouseup occurs outside.
    var _tolerate_mouseup = function (options,e) {
	if (options.tolerate_mouseup)
         	return options.tolerate_mouseup(e,options.context,refTime,delay);
        var now=new Date;
        return ( (now.getTime()-refTime.getTime())<delay && ((refX=e.pageX && refY==e.pageY) || $(event.target).closest('.jeegoocontext').size()==0))
    };
	...
	if (id=='reset') {
        ...
        $('#' + id).find('li')[_menus[id].livequery ? 'expire' : 'unbind']('.jeegoocontext')[_menus[id].livequery ? 'livequery' : 'bind']('mouseover.jeegoocontext', function(e){  
            ...
            e.stopPropagation(); 
        })[_menus[id].livequery ? 'livequery' : 'bind']('mouseup.jeegoocontext', function(e){

            if (_tolerate_mouseup(_menus[id],e))
    		return false;	    
	...
	return this[_menus[id].livequery ? 'expire' : 'unbind'](eventType)[_menus[id].livequery ? 'livequery' : 'bind'](eventType, function(e){
            // Check for the modifier if any.
			if (typeof _menus[id].modifier_disable == 'string' && e[_menus[id].modifier_disable]) return true;
			if (typeof _menus[id].modifier == 'string' && !e[_menus[id].modifier]) return false;
	    ...
	    refTime = new Date;
			...
			}).bind('mouseup.jeegoocontext', function(e){
				if (_tolerate_mouseup(_menus[id],e))
					return false;

				_globalHide.call(this);
			});
